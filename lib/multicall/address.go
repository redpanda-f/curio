...
func MultiCallAddress() (common.Address, error) {
	...
	case build.BuildLocalnet:
		multicallAddr := os.Getenv("FOC_LOCALNET_CONTRACT_MULTICALL")
		if multicallAddr == "" {
			return common.Address{}, xerrors.Errorf("FOC_LOCALNET_CONTRACT_MULTICALL environment variable must be set for localnet")
		}
		if !common.IsHexAddress(multicallAddr) {
			return common.Address{}, xerrors.Errorf("FOC_LOCALNET_CONTRACT_MULTICALL must be a valid hex address")
		}
